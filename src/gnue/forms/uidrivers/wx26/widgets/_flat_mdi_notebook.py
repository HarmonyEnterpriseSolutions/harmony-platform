...
#
# $Id: _flat_mdi_notebook.py,v 1.3 2013/12/03 19:58:32 Oleg Exp $
"""
"""
...
MdiNotebookClass = fnb.FlatNotebook
...
class UIMDINotebook (_base.UIHelper):
	...
	def _create_widget_ (self, event):
		...
		@returns: wx widget
		"""
		self._container = self.widget = fnb.FlatNotebook(event.container)
		# wx.WANTS_CHARS needed by cefpython and not works in constructor
		self._container.SetWindowStyleFlag(0
			#| wx.NO_BORDER
			| fnb.FNB_X_ON_TAB
			| fnb.FNB_FF2
			| fnb.FNB_NO_X_BUTTON
		...
		self.widget.Bind(fnb.EVT_FLATNOTEBOOK_PAGE_CLOSING, self.__on_page_closing, self.widget)
		self.widget.Bind(fnb.EVT_FLATNOTEBOOK_PAGE_CHANGED, self.__on_page_changed, self.widget)
	...
	def __on_page_closing(self, event):
		container = event.GetEventObject().GetPage(event.GetSelection())
		if container._uiForm_ is not None:
			#rint "__on_page_closing", container._uiForm_._gfObject
			# call logic
			container._uiForm_._gfObject.close()
